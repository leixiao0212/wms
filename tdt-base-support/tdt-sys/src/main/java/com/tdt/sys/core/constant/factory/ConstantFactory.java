 ...
 */
package com.tdt.sys.core.constant.factory;
...
import com.tdt.sys.core.constant.state.MenuStatus;
import com.tdt.sys.core.log.LogObjectHolder;
import com.tdt.sys.modular.system.entity.*;
import com.tdt.sys.modular.system.mapper.*;
import com.tdt.sys.modular.system.service.PositionService;
...
public class ConstantFactory implements IConstantFactory {
     ...
     */
    @Override
    @Cacheable(value = Cache.CONSTANT, key="#name")
    public List<Dict> getDictsByName(String name) {
        ...
        dictType = dictTypeMapper.selectOne(queryWrapper);
        if (dictType == null) {
    ...
    @Override
    @Cacheable(value = Cache.CONSTANT,key = "'map"  + "'+#name")
    public Map<String,String> getDictsByNames(String name) {
        DictType dictType = new DictType();
        dictType.setName(name);
        QueryWrapper<DictType> queryWrapper = new QueryWrapper<>(dictType);
        dictType = dictTypeMapper.selectOne(queryWrapper);
    ...
    @Override
    @Cacheable(value = Cache.CONSTANT, key="#name+'-'+#code")
    ...
    @Override
    @Cacheable(value = Cache.CONSTANT, key = "'" + CacheKey.DICT_CODE_LIST + "'+#code")
    public List<Dict> queryDictByCode(String code) {
        if (ToolUtil.isEmpty(code)) {
            return null;
        } else {
            DictType dictType = dictTypeMapper.selectOne(new QueryWrapper<DictType>().eq("code",code));
            if (dictType == null) {
                return null;
            } else {
                List<Dict> dicts = dictMapper.selectList(new QueryWrapper<Dict>().eq("dict_type_id", dictType.getDictTypeId()));
                return dicts;
            }
        }
    }
    ...
    @Override
    public String getPurchaseStateName(String state) {
        if (state.equals("0")) {
            return "未审核";
    ...
    @Override
    public List<Long> queryWarehousesByDept(Long dept_id){
        return this.deptMapper.queryWarehouses(dept_id);
    }
}
